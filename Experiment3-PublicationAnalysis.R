# code for publication. used to generate figures related to experiment 3
...
head.dir<-"/Users/anew/Downloads/HarmoniousCombinations-master/Experiment3"
...
###################################################
DTX<-data.table(DT0[,lapply(.SD,scale),.SDcols= notin(phenocols,'phenotypic.index')],phenDT[,genos,with=F])#copy(phenDT)
summ<-summary.func.all(DTX,notin(phenocols,'phenotypic.index'),genos)
# detection of slight growth rate defect when expressing high GAL pathway in the GAL1pr-HIS strain
...
p0<-ggplot(DT1[pheno=='growth.rate'],aes(mean.x,mean.y,xmax=upper.x,xmin=lower.x,ymin=lower.y,ymax=upper.y))+
