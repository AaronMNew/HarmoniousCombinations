...
load(paste0(outputdata,'1707xx-181024-SingleClustersToDoubles-Landscapes.rData'));load(paste0(outputdata,'1707xx-181024-SingleClustersToDoubles-Landscapes-ArrowFrame.rData'))
mutpair<-'GAL3 GAL80'
subo<-CloneNamedSummary(summs[faceting=='constitutive + uninducible = inducible',!c('mutant_gene','single_lv')],allele=F)
suba<-subo[mutant_gene==mutpair]
ccs<-unique(suba$cc);bbs<-unique(suba$bb);aas<-unique(suba$aa)
DT<-subo[cc%in%ccs|bb%in%bbs|clone.named=='GAL3.WT GAL80.WT GAL4.WT']
DT[,fac2:='G3VG80']
...
suba<-subo[mutant_gene==mutpair]
ccs<-unique(suba$cc);bbs<-unique(suba$bb);aas<-unique(suba$aa)
DT1<-subo[cc%in%ccs|aa%in%bbs|clone.named=='GAL3.WT GAL80.WT GAL4.WT']
DT1[,fac2:='G80VG4']
