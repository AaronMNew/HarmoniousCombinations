...
constclones<-c('GAL3.WT GAL80.34 GAL4-L868K','GAL3.WT GAL80.34 GAL4-L868F','GAL3.WT GAL80S-0 GAL4-L868E','GAL3.WT GAL80S-0 GAL4-L868K','GAL3.WT GAL80S-0 GAL4-L868F','GAL3.WT GAL80S-2 GAL4-L868K','GAL3.WT GAL80S-2 GAL4-L868F','GAL3.33 GAL80.25 GAL4.WT','GAL3.WT GAL80.03 GAL4-L868F','GAL3.WT GAL80.43 GAL4-L868E','GAL3.WT GAL80.43 GAL4-L868K','GAL3.WT GAL80.43 GAL4-L868P','GAL3.WT GAL80.34 GAL4-L868P','GAL3.WT GAL80S-0 GAL4-L868P','GAL3.WT GAL80S-2 GAL4-L868P','GAL3.24 GAL80.WT GAL4-L868E','GAL3.24 GAL80.WT GAL4-L868K','GAL3.24 GAL80.WT GAL4-L868P')
DTm[clone.named%in%constclones,cluster_new:='constitutive']
uninclones<-c('GAL3.12 GAL80.34 GAL4.WT',"GAL3.13 GAL80.WT GAL4.29","GAL3.06 GAL80.34 GAL4.WT","GAL3.08 GAL80.15 GAL4.WT","GAL3.09 GAL80.10 GAL4.WT","GAL3.09 GAL80.15 GAL4.WT","GAL3.09 GAL80.22 GAL4.WT","GAL3.20 GAL80.33 GAL4.WT",'GAL3.09 GAL80.15 GAL4.WT','GAL3.20 GAL80.33 GAL4.WT', "GAL3.WT GAL80.34 GAL4-L868F","GAL3.WT GAL80S-0 GAL4-L868F","GAL3.WT GAL80S-2 GAL4-L868F")
DTm[clone.named%in% uninclones,cluster_new:='uninducible']
indclones<-c('GAL3.WT GAL80.29 GAL4-L868C','GAL3.WT GAL80.29 GAL4.18','GAL3.WT GAL80.29 GAL4.19')
DTm[clone.named%in% indclones,cluster_new:='inducible']
...
if(writeplot==T){
	save(summs,file=paste0(outputdata,'1707xx-181024-SingleClustersToDoubles-Landscapes.rData'))
	save(arrows,file=paste0(outputdata,'1707xx-181024-SingleClustersToDoubles-Landscapes-ArrowFrame.rData'))
}
load(paste0(outputdata,'1707xx-181024-SingleClustersToDoubles-Landscapes.rData'));load(paste0(outputdata,'1707xx-181024-SingleClustersToDoubles-Landscapes-ArrowFrame.rData'))
...
pOKJG<-ggplot(summs1,aes(fracon.glu_mean,fracon.gal_mean,xmin=fracon.glu_lower,xmax=fracon.glu_upper,ymin=fracon.gal_lower,ymax=fracon.gal_upper))+
	...
	geom_point(aes(col= `mutation\norder`,shape= genes,alpha=alphas),size=ptsize,stroke=strokesize)+geom_abline(col='grey70')+
...
if(writeplot==T){	
	w<-8.5;h<-8.5
	ggsave(paste0(figout,'1707xx-181031-pOKJG-SingleClustersToDoubles-Landscapes.png'), pOKJG,width=w,height=h)
	}

# look at weirdly categorized clones
combo<-'inducible + constitutive = inducible'
clones1<-unique(summs[faceting%in%combo]$clone.named)
clones2<-c(phenDT[,mean(fracon.gal,na.rm=T)>0.5,by=genos][V1==T&clone.named%in%clones1]$clone.named,'GAL3.09 GAL80.WT GAL4.WT','GAL3.20 GAL80.WT GAL4.WT')
...
mDT1[density_mean>=max(ylimits),density_mean:=max(ylimits)][order(sugar,AU.FL,decreasing=T)]
unique(mDT1[single_lv==F]$clone.named)
if(writeplot==T){
...
ccs<-c(unique(phenDT$cc)[grepl('S',unique(phenDT$cc))],'GAL80.WT','GAL80.delta')
aas<-c(unique(phenDT$aa)[grepl('L868',unique(phenDT$aa))],'GAL4.WT','GAL4.delta')
# ccs<-unique(suba$cc);bbs<-unique(suba$bb);aas<-unique(suba$aa)

qfun<-function(phenosx){	
	DT0<-summary.func.all(phenDT,phenosx,bylist='clone.named')
	setnames(DT0,grepincols(DT0,phenosx[1]),gsub(phenosx[1],'x',grepincols(DT0,phenosx[1])))
	setnames(DT0,grepincols(DT0,phenosx[2]),gsub(phenosx[2],'y',grepincols(DT0,phenosx[2])))
	DT1<-merge(CloneNamedSummary(summary.func.all(phenDT[cc%in%ccs&aa%in%aas&bb=='GAL3.WT'|clone.named=='GAL3.WT GAL80.WT GAL4.WT'],phenosx,bylist='clone.named'),allele=F),clusterframe3,by=genos)
	dum<-data.table(mutcount=c(0,1,2),`mutation\norder`=factor(c('WT','single','double'),levels=c('WT','single','double')))
	DT1<-merge(DT1,dum,by='mutcount')
	DT1[,genes:=factor(gsub('GAL3 GAL80 GAL4','WT',mutant_gene),levels=c('WT','GAL4','GAL80','GAL80 GAL4'))]
	WT<-DT1[`mutation\norder`=='WT',c(grepincols(DT1,'_mean')),with=F]
	colnames(WT)<-c('x0','y0')
	sings<-DT1[`mutation\norder`=='single',c(grepincols(DT1,'_mean'),'mutants'),with=F]
	sings0<-copy(sings)
	sings0[,c('x0','y0'):=WT]
	sings0[grepl('GAL4',mutants),aa:=mutants]
	sings0[grepl('GAL80',mutants),cc:=mutants]
	sings0[is.na(aa),aa:='GAL4.WT']
	sings0[is.na(cc),cc:='GAL80.WT']
	setnames(sings0, c(grepincols(DT1,'_mean')),c('xend','yend'))
	sings1<-copy(sings)
	colnames(sings1)<-c('xend','yend','aa')
	sings1[,cc:=aa]
	sings2<-copy(sings1)
	setnames(sings2,c('xend','yend'),c('x0','y0'))
	doubs<-DT1[`mutation\norder`=='double',c(grepincols(DT1,'_mean'),'aa','cc','mutants'),with=F]
	setnames(doubs,c(grepincols(DT1,'_mean')),c('xend','yend'))
	a<-merge(doubs[,!'cc'],sings2[,!'cc'],by=c('aa'))
	b<-merge(doubs[,!'aa'],sings2[,!'aa'],by=c('cc'))
	arrows<-rbind(a[,!'aa'],b[,!'cc'])
	arrows[,c('cc','aa'):=colsplit(mutants,'\\ ',c('cc','aa'))]
	arrows[,`mutation\norder`:='double']
	sings0[,`mutation\norder`:='single']
	DTx<-copy(DT1)
	setnames(DTx,grepincols(DT1,phenosx[1]),gsub(phenosx[1],'x',grepincols(DT1,phenosx[1])))
	setnames(DTx,grepincols(DT1,phenosx[2]),gsub(phenosx[2],'y',grepincols(DT1,phenosx[2])))
	return(list(DTx,DT0,arrows,rbind(arrows,sings0)))
	}
qfun2<-function(DTlist){	
	# keep having to repeat this. the code is really junky but makes OK figures at the end
	DT0<-DTlist[[2]]
	DTx<-DTlist[[1]]
	arrows<-DTlist[[4]]
	ab<-expand.grid(aas,ccs)
	colnames(ab)<-c('aa','cc')
	WTx<-data.table(ldply(lapply(1:nrow(ab),function(i)return(copy(DTx[genes=='WT'])))))
	WTx[,c('aa','cc'):=list(ab[,1],ab[,2])]
	DTx1<-rbind(WTx,DTx)
	aalev<-c("GAL4.WT",'GAL4.delta',"GAL4-L868*","GAL4-L868C","GAL4-L868F","GAL4-L868E","GAL4-L868S","GAL4-L868K","GAL4-L868P","GAL4-L868G")
	DTx1[,aa:=factor(aa,levels=aalev)]
	arrows[,aa:=factor(aa,levels=aalev)]
	cclev<-c("GAL80.WT",'GAL80.delta','GAL80S-0','GAL80S-1','GAL80S-2','GAL80S-V352E')
	DTx1[,cc:=factor(cc,levels=cclev)]
	arrows[,cc:=factor(cc,levels=cclev)]
	test1<-arrows[`mutation\norder`=='single'&grepl('WT',aa),.(xend,yend)]
	test2<-arrows[`mutation\norder`=='single'&grepl('WT',cc),.(xend,yend)]
	test1[,genespre:='GAL80']
	test2[,genespre:='GAL4']
	setnames(test1,c('xend','yend'),c('x0','y0'))
	setnames(test2,c('xend','yend'),c('x0','y0'))
	test<-rbind(test1,test2)
	arrows<-merge(test,arrows,by=c('x0','y0'),all=T)
	arrows$genes<-factor(arrows$genespre,levels=levels(DTx$genes))
	arrows$`mutation\norder`<-factor(arrows$`mutation\norder`,levels=levels(DTx1$`mutation\norder`))
	DTest<-rbind(DTx1,arrows,fill=T)
	return(DTest)
	}

phenosx<-c('yfp.mean.glu','growth.rate')
DTlist<-qfun(phenosx)
DTest<-qfun2(DTlist)
DT0<-DTlist[[2]]
...
ptsize<-4
strokesize<-1
custshapes<-c(1,2,4,0,2,5,6)
limitsx<-c(-5000,35000)
limitsy<-c(-.05,0.3)
sample(letters,4)
pNQZO<-ggplot(DTest[order(`mutation\norder`,decreasing=T)],aes(x_mean,y_mean,xmin=x_lower,xmax=x_upper,ymin=y_lower,ymax=y_upper))+
	theme_minimal()+
#	geom_abline(col='black')+
	geom_point(data=na.exclude(DT0),aes(x_mean,y_mean),inherit.aes=F,col='grey50',size=0.2,alpha=0.05)+
	geom_point(aes(col= `mutation\norder`,shape= genes),size=ptsize,stroke=strokesize)+
	geom_segment(aes(y=y0,yend=yend,x=x0,xend=xend,col=`mutation\norder`),size=0.7,arrow=arrow(type='closed',length=unit(0.5,'cm'),angle=15),alpha=0.5,inherit.aes=F)+
	geom_point(aes(x0,y0,shape=genes),inherit.aes=F,col= factorDefault[2],size=ptsize,stroke=strokesize)+
	geom_point(aes(x0,y0),inherit.aes=F,col=factorDefault[3],size=1.5,shape=21)+
	geom_errorbar(col='black')+geom_errorbarh(col='black')+
	scale_shape_manual(values=custshapes)+
	scale_color_manual(values=factorDefault)+
	ylim(limitsy)+
	xlim(limitsx)+
	ylab(expression(paste('average growth rate ',mu,' ',hr^2)))+xlab('mean YFP signal AU in glucose')+
	theme(axis.text.x=element_text(angle=30))


phenosx<-c('yfp.mean.glu','yfp.mean.gal')
DTlist<-qfun(phenosx)
DTest<-qfun2(DTlist)
DT0<-DTlist[[2]]
...
strokesize<-1
custshapes<-c(1,2,4,0,2,5,6)
limitsx<-c(-5000,55000)
limitsy<-c(-5000,55000)

pBINC<-ggplot(DTest[order(`mutation\norder`,decreasing=T)],aes(x_mean,y_mean,xmin=x_lower,xmax=x_upper,ymin=y_lower,ymax=y_upper))+
	theme_minimal()+
	geom_abline(col='black')+
	geom_point(data=na.exclude(DT0),aes(x_mean,y_mean),inherit.aes=F,col='grey50',size=0.2,alpha=0.05)+
	geom_point(aes(col= `mutation\norder`,shape= genes),size=ptsize,stroke=strokesize)+
	...
	ylim(limitsy)+
	xlim(limitsx)+
	ylab('mean YFP signal AU in galactose')+xlab('mean YFP signal AU in glucose')+
	theme(axis.text.x=element_text(angle=30))



phenosx<-c('fracon.glu','growth.rate')
DTlist<-qfun(phenosx)
DTest<-qfun2(DTlist)
DT0<-DTlist[[2]]
DT0<-DTlist[[2]]
...
pVBFM<-ggplot(DTest[order(`mutation\norder`,decreasing=T)],aes(x_mean,y_mean,xmin=x_lower,xmax=x_upper,ymin=y_lower,ymax=y_upper))+
	theme_minimal()+
#	geom_abline(col='black')+
	geom_point(data=na.exclude(DT0),aes(x_mean,y_mean),inherit.aes=F,col='grey50',size=0.2,alpha=0.05)+
	geom_point(aes(col= `mutation\norder`,shape= genes),size=ptsize,stroke=strokesize)+
	...
	ylim(limitsy)+
	xlim(limitsx)+
	ylab(expression(paste('average growth rate ',mu,' ',hr^2)))+xlab('fraction ON cells in glucose')+
	theme(axis.text.x=element_text(angle=30))


phenosx<-c('fracon.glu','fracon.gal')
DTlist<-qfun(phenosx)
DT0<-DTlist[[2]]
DTest<-qfun2(DTlist)
...
pFXHW<-ggplot(DTest[order(`mutation\norder`,decreasing=T)],aes(x_mean,y_mean,xmin=x_lower,xmax=x_upper,ymin=y_lower,ymax=y_upper))+
	theme_minimal()+
	geom_abline(col='black')+
	geom_point(data=na.exclude(DT0),aes(x_mean,y_mean),inherit.aes=F,col='grey50',size=0.2,alpha=0.05)+
	geom_point(aes(col= `mutation\norder`,shape= genes),size=ptsize,stroke=strokesize)+
	...
	ylim(limitsy)+
	xlim(limitsx)+
	ylab('fraction ON cells in galactose')+xlab('fraction ON cells in glucose')+
	theme(axis.text.x=element_text(angle=30))


phenosx<-c('phenotypic.index','growth.rate')
DTlist<-qfun(phenosx)
DTest<-qfun2(DTlist)
DT0<-DTlist[[2]]
DT0<-DTlist[[2]]
...
pWFJK<-ggplot(DTest[order(`mutation\norder`,decreasing=T)],aes(x_mean,y_mean,xmin=x_lower,xmax=x_upper,ymin=y_lower,ymax=y_upper))+
	theme_minimal()+
#	geom_abline(col='black')+
	geom_point(data=na.exclude(DT0),aes(x_mean,y_mean),inherit.aes=F,col='grey50',size=0.2,alpha=0.05)+
	geom_point(aes(col= `mutation\norder`,shape= genes),size=ptsize,stroke=strokesize)+
	...
	ylim(limitsy)+
	xlim(limitsx)+
	ylab(expression(paste('average growth rate ',mu,' ',hr^2)))+xlab('fraction ON cells in glucose + fraction ON cells in galactose')+
	theme(axis.text.x=element_text(angle=30))



# pNQZO+facet_grid(cc~aa)
# pWFJK+facet_grid(cc~aa)
# pFXHW +facet_grid(cc~aa)
# pVBFM+facet_grid(cc~aa)
# pBINC +facet_grid(cc~aa)
...
lmatblank[(a*ROWS+1):ROWS,seq(1,COLS*X,by=1)]<-3
lmatblank[1:ROWS,seq((COLS*X+1),COLS,by=1)]<-2

lmat<-as.matrix(lmatblank)
if(writeplot==T){
	w<-18.5;h<-9.5
	ggsave(paste0(figout,'1707xx-181102-pNQZO-YFPMeanGluVsGrowthRate.png'),
		do.call(grid.arrange,list(pNQZO+theme(legend.position='none'),pNQZO+facet_grid(cc~aa),pBLANK, layout_matrix=lmat)),
		width=w,height=h)

	ggsave(paste0(figout,'1707xx-181102-pBINC-YFPMeanGluVsYFPMeanGal.png'),
		do.call(grid.arrange,list(pBINC +theme(legend.position='none'), pBINC +facet_grid(cc~aa),pBLANK, layout_matrix=lmat)),
		width=w,height=h)

	ggsave(paste0(figout,'1707xx-181102-pVBFM-FraconGluVsGrowthRate.png'),
		do.call(grid.arrange,list(pVBFM +theme(legend.position='none'), pVBFM +facet_grid(cc~aa),pBLANK, layout_matrix=lmat)),
		width=w,height=h)

	ggsave(paste0(figout,'1707xx-181102-pFXHW-FraconGluVsFraconGal.png'),
		do.call(grid.arrange,list(pFXHW +theme(legend.position='none'), pFXHW +facet_grid(cc~aa),pBLANK, layout_matrix=lmat)),
		width=w,height=h)
		
	ggsave(paste0(figout,'1707xx-181102-pWFJK-PhenotypicIndexVsGrowthRate.png'),
		do.call(grid.arrange,list(pWFJK +theme(legend.position='none'), pWFJK +facet_grid(cc~aa),pBLANK, layout_matrix=lmat)),
		width=w,height=h)
...
arrows<-DTlist[[3]]
limits<-c(0,1)
ggplot(DTx[order(`mutation\norder`,decreasing=T)],aes(x_mean,y_mean,xmin=x_lower,xmax=x_upper,ymin=y_lower,ymax=y_upper))+
	theme_minimal()+geom_abline(col='black')+
	geom_point(data=na.exclude(DT0),aes(x_mean,y_mean),inherit.aes=F,col='grey50',size=0.2,alpha=0.3)+
	geom_segment(data= arrows,aes(y=y0,yend=yend,x=x0,xend=xend),size=0.7,col='black',arrow=arrow(type='closed',length=unit(0.5,'cm'),angle=15),alpha=0.3,inherit.aes=F)+
	geom_errorbar(col='grey70')+geom_errorbarh(col='grey70')+
	geom_point(aes(col= `mutation\norder`,shape= genes),size=ptsize,stroke=strokesize)+
	scale_shape_manual(values=custshapes)+
	scale_color_manual(values=factorDefault)+
...
ggplot(DTx,aes(fracon.glu_mean,fracon.gal_mean,xmin=fracon.glu_lower,xmax=fracon.glu_upper,ymin=fracon.gal_lower,ymax=fracon.gal_upper))+
	theme_minimal()+
	geom_errorbar(col='grey70')+geom_errorbarh(col='grey70')+
	geom_point(aes(col= `mutation\norder`,shape= genes),size=ptsize,stroke=strokesize)+geom_abline(col='grey70')+
	scale_shape_manual(values=custshapes)+
	scale_color_manual(values=factorDefault)+
#	geom_segment(data= arrows,aes(y=y0,yend=yend,x=x0,xend=xend),size=0.1,col='black',arrow=arrow(type='closed',length=unit(0.3,'cm')),inherit.aes=F)+
#	geom_segment(data= arrows,aes(y=y0,yend=yend,x=x0,xend=xend,col=col),size=0.5,inherit.aes=F)+
...
WT<-subo[clone.named=='GAL3.WT GAL80.WT GAL4.WT']
xWT<-WT$fracon.glu_mean
yWT<-WT$fracon.gal_mean
xendx<-
yendx<-pre[cluster_new==ABphen][1,3]
...
summs1<-CloneNamedSummary(summs[,.(clone.named,faceting)],allele=F)
dsplit<-split(summs1[faceting=='constitutive + uninducible = inducible'&single_lv==F],by='mutant_gene')
x<-dsplit[[1]]
aas<-unique(c(x$aa,'GAL4.WT'));bbs<-unique(c(x$bb,'GAL3.WT'));ccs<-unique(c(x$cc,'GAL80.WT'))
summs[(bb%in%bbs&cc%in%ccs&aa%in%aas)]

clonelist<-lapply(dsplit,function(x0){
	x<-CloneNamedSummary(x0,allele=F)
	
	return()
})
set1<-clonelist[[1]]
s1<-summs[clone.named%in%set1]
s2<-summs[clone.named%in%clonelist[[2]]]
