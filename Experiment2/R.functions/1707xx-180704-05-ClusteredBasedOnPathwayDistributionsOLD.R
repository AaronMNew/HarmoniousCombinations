...
#	print(ABframe[!is.na(V2)])
	...
	# # doublecheck you have the right clones
	...
	# mDT<-melt(DT1[,c('cluster_new','clone.named',dens.cols),with=F],id=c('cluster_new','clone.named'))
	# denscolconv(mDT)
	# mDT[,dummyline:=paste0('z',sugar)]
	# mDT[,pheno:='fluorescence']
	...
	# ybreaks<-c(0.0,0.1,0.2,0.3)
	# ylabs<-c('0.0',paste0(ybreaks[2:3]),paste0('>',max(ybreaks)))
	# mDT1<-mDT[,mean(value,na.rm=T),by=c('clone.named','dummyline','AU.FL','sugar')];setnames(mDT1,'V1','density_mean')
	# mDT1[density_mean>=max(ylimits),density_mean:=max(ylimits)][order(sugar,AU.FL,decreasing=T)]
	
	# if(writeplot==T){
		# ggplot(mDT1,aes(AU.FL,density_mean))+theme_minimal()+
			# geom_line(aes(col=dummyline),size=2)+geom_line(aes(col=sugar))+
			# facet_wrap(~clone.named,ncol=5)+scale_colour_manual(values=c('orange1','steelblue','grey90','grey90'),na.value='transparent')+
			# ylab('density') + xlab('pseudo-log10 fluorescence intensity, A.U.')+
			# scale_y_continuous(limits=ylimits,breaks=ybreaks,labels=ylabs)+
			# theme(legend.title=element_blank())+
			# theme(strip.text.x = element_text(size = 10))
...
ggplot(summs,aes(fracon.glu_mean,fracon.gal_mean,xmin=fracon.glu_lower,xmax=fracon.glu_upper,ymin=fracon.gal_lower,ymax=fracon.gal_upper))+
	theme_minimal()+
	geom_errorbar(col='grey70')+geom_errorbarh(col='grey70')+
	geom_point(aes(col= fac,shape= pairing),alpha=0.3,size=ptsize,stroke=strokesize)+geom_abline(col='grey70')+
	scale_shape_manual(values=custshapes)+
	scale_color_manual(values=factorDefault)+
	geom_segment(data= arrows,aes(y=y0,yend=yend,x=x0,xend=xend,col=col), arrow=arrow(type='closed',length=unit(0.3,'cm')),size=1,inherit.aes=F)+
	ylim(limits)+xlim(limits)+ylab('fraction ON cells in galactose')+xlab('fraction ON cells in glucose')+
	facet_wrap(~faceting,ncol=3)



p1<-ggplot(summ1,aes(fracon.glu_mean,fracon.gal_mean,xmin=fracon.glu_lower,xmax=fracon.glu_upper,ymin=fracon.gal_lower,ymax=fracon.gal_upper))+
	theme_minimal()+
	geom_errorbar(col='grey70')+geom_errorbarh(col='grey70')+
	geom_point(aes(col= fac,shape= pairing),size=ptsize,stroke=strokesize)+geom_abline(col='grey70')+
	scale_shape_manual(values=custshapes)+
	scale_color_manual(values=factorDefault)+
	geom_segment(data= arrowframe,aes(y=y0,yend=yend,x=x0,xend=xend),col='black', arrow=arrow(type='closed',length=unit(0.3,'cm')),size=1,alpha=0.5,inherit.aes=F)+
	ylim(limits)+xlim(limits)+ylab('fraction ON cells in galactose')+xlab('fraction ON cells in glucose')+
	facet_wrap(~faceting)
