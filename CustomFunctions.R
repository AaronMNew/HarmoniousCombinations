...
###################################################
### custom pushing functions (grepcols, grepincols etc)
###################################################

grepcols<-function(x,fac){
    # for quickly pulling data tables out of other data tables based on grepping colnames
    # x is a data frame or table with column set containing characters that will be matched with the character(s) in fac
    outlist<-list()
    len<-0
    nameset<-list()
    for(i in 1:length(fac)){
        outlist[[i]]<-x[,grep(fac[i],colnames(x)),with=FALSE]
        names(outlist[i])<-fac[i]
        len<-len+length(outlist[[i]])
        nameset[[i]]<-names(outlist[[i]])
    }
    output<-data.table(matrix(unlist(outlist),ncol=len,byrow=F))
    colnames(output)<-unlist(nameset)
    output
# VALIDATION
# # DT.dens' dens.cols is the example thing here -- checking that grepcols returns the same data as standard subsetting
# x<-grepcols(DT.dens,c(dens.cols))
# y<-DT.dens[,dens.cols,with=FALSE]
# (sapply(colnames(x),function(i){prod(as.numeric(na.exclude(x[,i,with=FALSE]==y[,i,with=FALSE]) ))}))
...
grepnumericcols<-function(X){
    xa<-data.frame(X)
    numerics<-list()
    for(i in 1:ncol(xa)){
        numerics[[i]]<-is.numeric(xa[,i])
    }
    data.table(xa[,unlist(numerics)])
...
zscore<-function(xdf){
	as.numeric((xdf-(mean(na.exclude(xdf))))/sd(na.exclude(xdf)))}

row.normalization<-function(x){
	cumulative<-sum(x,na.rm=TRUE)
	as.numeric(x)/cumulative
}

summary.func<-function(x,name=NULL){
    xa<-na.exclude(x)
    mu<-mean(xa)
    stdev<-sd(xa)
    N<-length(xa)
    CoV<-abs(stdev/mu)
    upper<-mu+(stdev/sqrt(N))*1.96
    lower<-mu-(stdev/sqrt(N))*1.96
	se<-stdev/sqrt(N)
	se_cov<-se/mu
    a<-list(mu,stdev,CoV,upper,lower,N,se,se_cov)
    names(a)<-c('mean','sd','CoV','upper','lower','N','se','SE_cov')
    if(!is.null(name))names(a)<-unlist(lapply(names(a),function(x){paste0(name,'_',x,collapse='')}))
    a
}

t.test2 <- function(m1,m2,s1,s2,n1,n2,m0=0,equal.variance=FALSE,list=F){
...
# this is Old, replaced by merge_recurse3 around 180701. eventually move to graveyard.
	x<-qDT
...
all.ttest<-function(DTx,pheno='pheno',bylist,padj='fdr',sig.cutoff=0.05){
	# DTx<-copy(phenDT)
	# pheno <- 'growth.rate'
	...
	return(data.table(DT6))
}



ttests.plot<-function(DT,padj='fdr',sig.cutoff=0.05,points=T){
	# requires out data.table generated by all.ttest() function
	out<-copy(DT)
	out[,sig:=p.adj<=sig.cutoff]
	lims<-c(-max(abs(range(out $diff))),max(abs(range(out $diff))))
	signame<-paste0(padj,' p < ', sig.cutoff)
	diffname<-'Var1 - Var2'
	
	p<-ggplot(out,aes(Var1,Var2,fill=diff))+geom_tile()+
		scale_fill_gradientn(colours=c('cornflowerblue','white','indianred'),name=diffname,limits=lims)+
		theme(axis.text.x=element_text(angle=30,hjust=01))+
		theme(axis.text.x=element_text(size=8),axis.text.y=element_text(size=8))
	
	if(points==T)p<-p+geom_point(aes(col=sig))+scale_colour_manual(values=c('orange1','darkgreen'),name=signame)#+labs(value= signame)
	return(p)
}


TukeyInteractionFlip2<-function(df,plot=T,points=T){
	...
	out<-merge(dum,tri_lv_frame,by=c('X1','X2'))[value==T][,!c('X1','X2','value')]
	out[,`p adj < 0.05`:=sig]
	# calculate limits for plotting colours
	lims<-c(-max(abs(range(out $`diff A-B`))),max(abs(range(out$`diff A-B`))))
	# make plot
	p<-ggplot(out,aes(A,B,fill=`diff A-B`))+geom_tile()+scale_fill_gradientn(colours=c('cornflowerblue','white','indianred'),limits=lims)+theme(axis.text.x=element_text(angle=30,hjust=01))+
		theme(axis.text.x=element_text(size=8),axis.text.y=element_text(size=8))
	# output
	if(points==T)p<-p+geom_point(aes(col=`p adj < 0.05`))+scale_colour_manual(values=c('orange1','darkgreen'))
	if(plot==T)return(p)
...
TukeyInteractionFlip<-function(df){
	# old code, don't use
	# df must be one of the data.frames from an object generated by the 'aov' function
	# make A and B columns
	DTemp1<-na.exclude(data.table(df,data.table(colsplit(rownames(df),'-',c('A','B')))[,lapply(.SD,as.character)]))
...
pairs.spearm<- function(x2)
{# custom pairs function made by aaron 
		pairs(x2,pch=".", upper.panel=panel.cor.spearman, diag.panel=panel.hist40, lower.panel=panel.smooth)
}
pairs.pears<- function(x2,cexlog=F,cexsetting)
{# custom pairs function made by aaron 
	cexs=1
	if(cexlog)
		cexs<-cexsetting
	pairs(x2,pch=".", upper.panel= panel.cor.pearson, diag.panel=panel.hist40, lower.panel=panel.smooth,cex.labels=cexs)
}
...
panel.cor.pearson <- function(x, y, r=NULL, digits = 2, prefix = "", cex.cor,  ...)
{
	usr <- par("usr"); on.exit(par(usr))
	par(usr = c(0, 1, 0, 1))
	r <- round(cor(x, y, use="pairwise.complete.obs", method="pearson"),digits)
	r2 <- round(summary(lm(y~x))$r.squared,digits)
	if(r=='pears'){txt <- paste0(r)}else{txt <- paste0(r2)}
	if(missing(cex.cor)) cex.cor <- 0.8/strwidth(txt)
...
radial_plot_space<-function(DT,genos){
	...
	colnames(radiusID)<-paste0('r',nums[-1])
	radiusID[,r1:=c(TRUE,rep(FALSE,(nrow(radiusID)-1)))]
...
grid_arrange_shared_legend <- function(..., nrow = 1, ncol = length(list(...)), position = c("bottom", "right"),for.new.dev=T) {
